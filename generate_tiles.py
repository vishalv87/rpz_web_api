...
def save_tiles(tiles, chessboard_img_path):
    ...
    print(f"image save directory: {img_save_dir}")
...
def generate_tiles_from_random_chessboard():
    if not os.path.exists(TILES_DIR):
        os.makedirs(TILES_DIR)
    chessboard_img_filenames = glob("{}/*/*.png".format(CHESSBOARDS_DIR))
    random_img_filename = random.choice(chessboard_img_filenames)

    img_save_dir = _img_save_dir(random_img_filename)

    if os.path.exists(img_save_dir) and not OVERWRITE:
        return "\tIgnoring existing {}\n".format(img_save_dir)

    tiles = get_chessboard_tiles(random_img_filename, use_grayscale=USE_GRAYSCALE)
    if len(tiles) != 64:
        return "\t!! Expected 64 tiles. Got {}\n".format(len(tiles))

    tiles_folder_path = save_tiles(tiles, random_img_filename)

    img_file_name = random_img_filename.split(".")[1].split("/")[-1]
    zip_file_path = zip_files(
        tiles_folder_path, f"{tiles_folder_path}/{img_file_name}.zip"
    )
